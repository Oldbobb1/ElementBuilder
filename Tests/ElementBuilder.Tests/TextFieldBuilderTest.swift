...
final class TextFieldBuilderTest: XCTestCase {
    func testAppearencePropTextfield() {
        ...
        let testTextField = TextFieldBuilder.textField(testConfiguration, testAppearance)
        XCTAssertEqual(testTextField.placeholder, testConfiguration.placeholder)
        XCTAssertEqual(testTextField.textContentType, .name)
        XCTAssertEqual(testTextField.keyboardType, .default)
        XCTAssertEqual(testTextField.autocapitalizationType, .words)
        XCTAssertEqual(testTextField.spellCheckingType, .no)
        XCTAssertEqual(testTextField.enablesReturnKeyAutomatically, .random())
        XCTAssertEqual(testTextField.clearButtonMode, .whileEditing)
        XCTAssertEqual(testTextField.returnKeyType, .default)
        XCTAssertEqual(testTextField.backgroundColor, .systemGray6)
    ...
    func testStyleTextField() {
        ...
        let testTextField = TextFieldBuilder.textField(nil, nil, testStyle)
    ...
    func testCombinedValues() {
        ...
        let testTextField = TextFieldBuilder.textField(testConfiguration, testAppearance, testStyle)
        XCTAssertEqual(testTextField.placeholder, testConfiguration.placeholder)
        XCTAssertEqual(testTextField.textContentType, .name)
        XCTAssertEqual(testTextField.keyboardType, .default)
        XCTAssertEqual(testTextField.autocapitalizationType, .words)
