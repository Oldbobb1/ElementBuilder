...
public struct ButtonBuilder {
    ...
    public static func buttonSystemAndButtonImage(
        ...
        if let buttonAppearanceAttributes = buttonAppearanceAttributes {
            ...
            button.setTitleColor(buttonAppearanceAttributes.setTitleColor, for: .normal)
            button.titleLabel?.font = buttonAppearanceAttributes.font
            button.configuration = buttonAppearanceAttributes.configuration
            button.backgroundColor = buttonAppearanceAttributes.backgroundColor ?? .systemBackground
            button.layer.cornerRadius = buttonAppearanceAttributes.cornerRadius ?? 0
            button.clipsToBounds = buttonAppearanceAttributes.clipsToBounds ?? true
            button.contentHorizontalAlignment = buttonAppearanceAttributes.contentHorizontalAlignment ?? .center
        ...
        if let buttonImageAttributes = buttonImageAttributes {
            ...
            if let systemName = buttonImageAttributes.systemName {
                let image = buttonImageAttributes.setImage ?? UIImage(systemName: systemName)
                if let image = image {
                    let newSize = buttonImageAttributes.imageSize ?? CGSize(
